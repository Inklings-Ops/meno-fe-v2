...
class BibleNotifier extends StateNotifier<BibleState> {
  ...
  void updateBook(Book book) {
    state = state.copyWith(book: book, numberOfChapters: book.numberOfChapters);
  }

  /// Shows the number of chapters
  /// Example, for Genesis, it returns 50 numbers
  /// representing the 50 chapters of the book of Genesis
  Future<void> updateNumberOfChapters(int number) async {
  ...
  Future<void> init() async {
    ...
    final chapter = await _facade.getChapter(
      state.reference,
      state.translation,
    );
